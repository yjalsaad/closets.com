...
|---|---|---|
| 2026-07-26 | **Palette v3 — Summer "Sea Breeze".** Owner: "theme for summer, colours more fresh." Re-graded the warm editorial layer to a cool-airy summer base, token-first in `:root`. Canvas walnut-bone → sea-mist (`--cream #f1f5f4`, `--sand #e2ece9`, `--line #d6e0dd`); ink → cool pine-charcoal `#1b2a27`; added a fresh secondary `--teal #2F8C7E` (`--teal-deep #256F63`) used for eyebrows, links, focus ring; CTA clay brightened to fresh coral `--clay #C65D2E` (AA-safe on #fff). Fixed `body::before` daylight canvas moved warm→sea-mist with faint teal + coral corner washes. Hero background blobs repointed from AI-slop violet `#8b5cf6` / cyan `#06b6d4` → `--teal` / `--brass` (blob-1/4 already `--clay`), closing the palette-coherence gap. | Owner wanted a fresher, summery feel. Teal + coral is a complementary pair: teal carries "cool + airy," coral keeps brand energy on the CTA. Token-first so the whole site shifts at once; coral CTA re-checked ≥4.5:1 on white; teal reserved for small accents/eyebrows. Removing the violet/cyan blobs ends a long-standing clash with the warm-luxury identity. |
