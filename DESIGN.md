...
|---|---|---|
| 2026-07-26 | **Palette v3 — Summer "Sea Breeze".** Owner: "theme for summer, colours more fresh" → then "change the green with very light orange." Re-graded the warm editorial layer to a cool-airy summer base, token-first in `:root`. Canvas walnut-bone → sea-mist (`--cream #f1f5f4`, `--sand #e2ece9`, `--line #d6e0dd`); ink → cool pine-charcoal `#1b2a27`; CTA clay brightened to fresh coral `--clay #C65D2E` (AA-safe on #fff). Secondary accent is a **light orange** `--peach #F2A566` (`--peach-deep #E08A44`) used for decorative washes / hero glow / rules only — NOT small text (too pale for AA). Eyebrows + focus ring stay on readable coral `--clay`. Fixed `body::before` daylight canvas is sea-mist with a light-orange + coral corner wash. Hero background blobs repointed from AI-slop violet `#8b5cf6` / cyan `#06b6d4` → `--peach` / `--brass` (blob-1/4 already `--clay`), closing the palette-coherence gap. | Owner wanted a fresher, summery feel, warm not green. Coral + light orange on a cool sea-mist canvas = warm accents popping on a cool ground. Token-first so the whole site shifts at once; coral re-checked ≥4.5:1 on white; light orange kept to decorative roles because a very light orange fails text contrast. Removing the violet/cyan blobs ends a long-standing clash with the brand. |
| 2026-07-26 | Global premium canvas: fixed `body::before` daylight layer + `.card` resting shadow + `.band-soft`/`.panel-soft` utilities | Transparent content sections previously showed flat `#fff`; a fixed lit-from-above wash makes white/sand sections read as floating panels (blend of daylight + bands + panels). |
